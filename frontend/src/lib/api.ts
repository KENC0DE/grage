...
export const subcategoriesAPI = {
  ...
  getByCategory: async (categoryId: number) => {
    const response = await api.get<
      ApiResponse<{ subcategories: Subcategory[]; category: Category }>
    >(`/categories/${categoryId}/subcategories`);
    return response.data;
  },

  getById: async (id: number) => {
    const response = await api.get<ApiResponse<{ subcategory: Subcategory }>>(
      `/subcategories/${id}`,
    );
    return response.data;
  },
...
export const serviceRequestsAPI = {
  getAll: async (params?: {
    ...
    const response = await api.get<
      ApiResponse<{ serviceRequests: ServiceRequest[] }>
  ...
  getNearbyGarages: async (id: number, maxDistance?: number) => {
    const response = await api.get<
      ApiResponse<{ garages: User[]; serviceRequest: any }>
    >(`/service-requests/${id}/nearby-garages`, {
      params: { maxDistance },
    });
    return response.data;
  },
